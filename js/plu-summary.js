...
async function loadDocument() {
  try {
    ...
    // Update title
    documentTitle.textContent = `Synthèse du PLU de ${document.ville.nom} - ${document.zonage.nom} - ${document.zone.nom}`;

    // Render markdown content
    const htmlContent = marked(document.plu_summary_markdown_content);
    ...
    if (document.pdf_storage_path) {
      ...
      downloadBtn.onclick = () => downloadPDF(document.pdf_storage_path);
    }
...
function generateTableOfContents() {
  const headings = pluContent.querySelectorAll("h1, h2, h3, h4");
